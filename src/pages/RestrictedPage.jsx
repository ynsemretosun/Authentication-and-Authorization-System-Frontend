...
function RestrictedPage() {
  // İşlemin durumu ve mesajın alınması için state'lerin tanımlanması
  const [isLoading, setIsLoading] = useState(true);
  const [message, setMessage] = useState("");
  const navigate = useNavigate();
  // Kullanıcının yetkili sayfaya erişip erişemediğinin kontrolü
  useEffect(() => {
    // Kısıtlı sayfaya erişim sağlayan fonksiyon
    const fetchRestricted = async () => {
      try {
        ...
        // Eğer kullanıcı kısıtlanmış sayfaya erişebiliyorsa mesajı al
        if (response.ok) {
  ...
  // Eğer işlem devam ediyorsa spinner göster
  if (isLoading) return <Spinner fullScreen={true} />;
  // Eğer mesaj yoksa yani kullanıcı kısıtlanmış sayfaya erişemiyorsa hata sayfasına yönlendir
  if (!message)
    return (
      <ErrorPage
        code={403}
